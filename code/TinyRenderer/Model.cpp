...
std::vector<vec3> Model::getFace(int idx){
    std::vector<vec3> ret;
    for(int i=0;i<3;i++){
        ret.push_back(vert(idx,i));
    }
    return ret;
}

std::vector<vec3> Model::getVn(int idx){
    std::vector<vec3> ret;
    for(int i=0;i<3;i++){
        ret.push_back(normal(idx,i));
    }
    return ret;
}

std::vector<vec2> Model::getVt(int idx){
    std::vector<vec2> ret;
    for(int i=0;i<3;i++){
        ret.push_back(uv(idx,i));
    }
    return ret;
}
...
TGAColor Model::diffuse2(const vec2 &uvf) const {
    TGAColor c = diffusemap_.get(uvf[0]*diffusemap_.get_width(), uvf[1]*diffusemap_.get_height());
    return c;
}
